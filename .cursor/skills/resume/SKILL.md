...
description: >-
  ...
  folder with job notes, and summarizes findings in chat. Use when the user
  pastes or links a job description, asks for a tailored resume for a new
...
- Keep research light at apply time: enough for a good résumé and short chat notes. Deeper interview prep belongs in the **interview-swot** skill.
...
1. **Add** `src/data/resumes/<slug>.ts` exporting `<camelCase>Resume: Resume` with:
   ...
   - **`about`** — **Scannable SUMMARY** a hiring manager can skim in ~15 seconds: aim for **~80–140 words** in **2–3 short paragraphs** (shorter is fine when the user asks), separated by a **blank line** in the string so the renderer prints distinct blocks; **lead with role fit**, then proof—leave granular proof in **EXPERIENCE**. Write in **plain, human language** about what you shipped and why you fit—**no insider mash-ups** or unexplained shorthand.

     **No “ramp” / learning-gap copy (mandatory):**
     - **Do not** put on the résumé (in `about`, skills, or experience) that a tool, language, or platform is a “ramp,” “honest gap,” “not headline tenure,” “would learn,” “transfer from X,” or similar. The user can learn stack specifics; résumés sell fit and shipped work, not a syllabus of what to pick up.
     - **Do not** invent caveats like “I have not used X in production” unless the user explicitly asks for that honesty on that résumé.
     - Mirror the posting’s stack confidently when it is reasonable given `base-profile` (e.g. MySQL alongside Postgres depth; Fastify alongside Node APIs). Prefer positive overlap over self-disclaimers.
   ...
   - **`skills`** — Start from `baseSkills`; add/reorder categories or items to mirror the posting (e.g. payments, CMS, AI). Keep honest: don’t invent skills. **Do not** add skill bullets that apologize for or “ramp” a named tool from the JD.
...
- **PDF:** `prospects/<FolderName>/resumes/Shenstone, Andrew - <Role> @ <Company>.pdf` (auto-export via `pnpm export-pdf`; manual Print → Save as PDF is fallback)
```
...
- **Writing SWOT at apply time** — Do not create `swot.md` or a SWOT section unless the user is interview-prepping (interview-swot skill) or explicitly asks.
- **“Ramp” / gap disclaimers on the résumé** — Saying a JD tool is a ramp, not headline tenure, or something you’d learn. Sell shipped overlap; leave learning assumptions out of PDF copy.
